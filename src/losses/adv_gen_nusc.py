...
class AdvGenLoss(nn.Module):
    ...
    def __init__(self, loss_weights, veh_att, mapixes, map_env, init_z, ptr,
                    ...
                    # 运动学可行性约束参数（背景车辆 vs 攻击车辆使用不同上限）
                    kine_v_max_veh=20.0,    # m/s，背景车辆速度上限
                    kine_a_max_veh=5.0,     # m/s²，背景车辆加速度上限
                    kine_j_max_veh=10.0,    # m/s³，背景车辆 jerk 上限
                    kine_v_max_atk=30.0,    # m/s，攻击车辆速度上限（放宽）
                    kine_a_max_atk=9.0,     # m/s²，攻击车辆加速度上限（放宽，~0.9g）
                    kine_j_max_atk=18.0,    # m/s³，攻击车辆 jerk 上限（放宽）
                    # THW 损失参数
                    thw_cap=4.0,            # s，超过此值不提供梯度（攻击者过远）
                    thw_danger=1.5,         # s，低于此值已建立危险前提，损失归零
                    ):
        '''
        :param loss_weights: dict of weightings for loss terms
        ...
        :param kine_*:      运动学约束上限，veh 为背景车辆，atk 为攻击车辆（放宽）
        :param thw_cap:     THW 梯度截止阈值（秒），超过此值损失为常数（见 THWLoss）
        :param thw_danger:  THW 危险阈值（秒），低于此值损失归零（见 THWLoss）
        '''
        ...
        # THW 损失（与 TTC 互补：速度相近时 TTC=∞无梯度，THW 依然有效）
        self.thw_loss = THWLoss(dt=dt, thw_cap=thw_cap, thw_danger=thw_danger, epsilon=ttc_epsilon)
        self.veh_att = veh_att  # 存储车辆尺寸，用于计算横向距离

        # 运动学可行性软约束（背景车辆：严格上限；攻击车辆：放宽上限）
        self.kine_loss_veh = KinematicFeasibilityLoss(
            v_max=kine_v_max_veh, a_max=kine_a_max_veh, j_max=kine_j_max_veh, dt=dt
        )
        self.kine_loss_atk = KinematicFeasibilityLoss(
            v_max=kine_v_max_atk, a_max=kine_a_max_atk, j_max=kine_j_max_atk, dt=dt
        )
        self.dt = dt
    ...
    def forward(self, future_pred, tgt_traj, z, prior_out,
        ...
        if 'adv_crash' in self.loss_weights and self.loss_weights['adv_crash'] > 0.0:
            ...
            if attack_agt_idx is not None:
                NA_loss = future_pred.size(0)
                valid_idx = attack_agt_idx[(attack_agt_idx >= 0) & (attack_agt_idx < NA_loss)]
                if valid_idx.numel() > 0:
                    attack_agt_mask = torch.zeros(NA_loss, dtype=torch.bool).to(valid_idx.device)
                    attack_agt_mask[valid_idx] = True
                    attack_agt_mask = attack_agt_mask[~self.ego_mask].unsqueeze(1).expand_as(min_dist_in)
                    min_dist_in = torch.where(~attack_agt_mask, float('inf')*torch.ones_like(min_dist_in), min_dist_in)
        ...
        # 如果指定了攻击车辆，创建mask来筛选（过滤越界索引）
        attacker_mask_for_loss = None
        if attack_agt_idx is not None:
            NA_loss = future_pred.size(0)
            valid_idx = attack_agt_idx[(attack_agt_idx >= 0) & (attack_agt_idx < NA_loss)]
            if valid_idx.numel() > 0:
                full_attack_mask = torch.zeros(NA_loss, dtype=torch.bool).to(future_pred.device)
                full_attack_mask[valid_idx] = True
        ...
        # THW 损失（与 TTC 互补）：仅对攻击者在 ego 前方且 THW 未达危险阈值时有梯度
        thw_loss_val = None
        if 'thw' in self.loss_weights and self.loss_weights['thw'] > 0.0 and attacker_pred.numel() > 0:
            thw_loss_val = self.thw_loss(attacker_pred, tgt_expanded_for_pairs)
            if torch.isnan(thw_loss_val).any():
                thw_loss_val = torch.where(
                    torch.isnan(thw_loss_val),
                    torch.zeros_like(thw_loss_val),
                    thw_loss_val,
                )

            if attacker_mask_for_loss is not None:
                # [NA-B, T-1] — 只计算指定攻击车辆
                mask_expanded = attacker_mask_for_loss.unsqueeze(1).expand_as(thw_loss_val)
                thw_loss_val_masked = thw_loss_val * mask_expanded.float()
                if attacker_mask_for_loss.sum() > 0:
                    loss = loss + self.loss_weights['thw'] * \
                        thw_loss_val_masked.sum() / mask_expanded.sum()
                else:
                    loss = loss + self.loss_weights['thw'] * thw_loss_val.mean()
            else:
                loss = loss + self.loss_weights['thw'] * thw_loss_val.mean()
            loss_out['thw_loss'] = thw_loss_val
        ...
        # ── 运动学可行性约束（防止极端加速度/速度/Jerk 产生"捷径解"）─────────
        has_kine_veh = 'kine_veh' in self.loss_weights and self.loss_weights['kine_veh'] > 0.0
        has_kine_atk = 'kine_atk' in self.loss_weights and self.loss_weights['kine_atk'] > 0.0

        if has_kine_veh or has_kine_atk:
            # kine_loss_val_all: [NA] — 对全体车辆计算（ego + non-ego），按需分拆
            # 注：此处共用同一 traj，ego 的约束由 kine_veh weight 管控
            non_ego_traj = future_pred[~self.ego_mask]   # [NA-B, T, 4]

            kine_loss_contrib = 0.0

            if has_kine_veh and non_ego_traj.numel() > 0:
                # 背景非ego车辆（排除攻击者）
                kine_val_all = self.kine_loss_veh(non_ego_traj)  # [NA-B]
                if attacker_mask_for_loss is not None:
                    bg_mask = ~attacker_mask_for_loss
                    if bg_mask.sum() > 0:
                        kine_loss_contrib = kine_loss_contrib + \
                            self.loss_weights['kine_veh'] * kine_val_all[bg_mask].mean()
                else:
                    kine_loss_contrib = kine_loss_contrib + \
                        self.loss_weights['kine_veh'] * kine_val_all.mean()
                loss_out['kine_veh_loss'] = kine_val_all

            if has_kine_atk and non_ego_traj.numel() > 0:
                # 攻击者（放宽上限）
                kine_val_atk = self.kine_loss_atk(non_ego_traj)  # [NA-B]
                if attacker_mask_for_loss is not None and attacker_mask_for_loss.sum() > 0:
                    kine_loss_contrib = kine_loss_contrib + \
                        self.loss_weights['kine_atk'] * kine_val_atk[attacker_mask_for_loss].mean()
                else:
                    kine_loss_contrib = kine_loss_contrib + \
                        self.loss_weights['kine_atk'] * kine_val_atk.mean()
                loss_out['kine_atk_loss'] = kine_val_atk

            loss = loss + kine_loss_contrib
...
class TTCLossAtk(nn.Module):
    """
    Adversarial TTC loss: directly returns TTC values so that gradient
    descent *minimises* TTC (pushes trajectories closer to collision).
    Infinite TTC (no collision risk) is clamped to `ttc_cap` to provide
    a finite but large gradient signal.
    """
    def __init__(self, epsilon=1e-6, dt=0.1, ttc_cap=8.0):
        super(TTCLossAtk, self).__init__()
        self.epsilon = epsilon
        self.dt = dt
        self.ttc_cap = ttc_cap

    def forward(self, future_pred, tgt_traj):
        _, _, ttc = compute_longitudinal_metrics(
            future_pred, tgt_traj, dt=self.dt, epsilon=self.epsilon
        )
        ttc = torch.where(torch.isinf(ttc), torch.full_like(ttc, self.ttc_cap), ttc)
        return ttc
...
class THWLoss(nn.Module):
    """
    Time Headway 攻击损失 — 鼓励攻击者维持对 ego 的近距纵向跟随，
    为急刹/无征兆制动等纵向攻击场景建立危险的"前提条件"。

    与 TTC 的互补关系
    ─────────────────
    TTC = d_lon / |v_ego - v_atk|  仅当 ego 比攻击者快（正在接近）时才提供梯度。
    THW = d_lon / v_ego             只要攻击者在 ego 正前方即有梯度；
                                    速度相近时 TTC = ∞（梯度=0），THW 依然有效。

    典型使用场景
    ─────────────
    SuddenBrakingUnpredictable（前车无征兆急刹）：
      - 优化初期：攻击者与 ego 同速行驶，TTC = ∞ 无梯度
        THW 持续鼓励攻击者保持近距跟随（建立危险前提）
      - 优化后期：攻击者开始制动，TTC 接管主导梯度

    损失设计
    ─────────
    仅当 ego 仍有充足反应距离（THW > thw_danger）时施加惩罚；
    一旦 THW 已降至危险阈值以下，损失归零（目标已达成）。
    攻击者在 ego 身后时损失同样为零（方向不对，无需驱动）。

    输出结构: [NA-B, T-1]，与 TTCLossAtk 形状一致，
    梯度下降最小化此值 → 攻击者贴靠 ego 正前方。

    推荐权重范围: 1.0–4.0（弱于 ttc，作辅助建立前提条件）。
    纵向攻击场景（SuddenBraking 系列）: 2.0–3.5
    横向/路口攻击场景: 0.5–1.0 或不启用（攻击者未必在前方）
    """

    def __init__(
        self,
        dt: float = 0.1,
        thw_cap: float = 4.0,       # s — 超过此阈值攻击者太远，停止提供梯度
        thw_danger: float = 1.5,    # s — 低于此阈值已达危险水平，损失归零
        epsilon: float = 1e-6,
    ):
        """
        :param dt:          时间步长（秒），与整个损失体系保持一致
        :param thw_cap:     THW 上限（秒）。超过此值时不再惩罚（攻击者距 ego 过远，
                            无法构成近距跟随场景）。默认 4.0s。
        :param thw_danger:  THW 危险阈值（秒）。低于此值视为已建立危险前提，
                            损失归零以避免过度优化。基于安全驾驶标准：城市场景
                            < 1.5s 属于高风险跟车距离。默认 1.5s。
        :param epsilon:     数值稳定性常数，防止除零。
        """
        super(THWLoss, self).__init__()
        self.dt = dt
        self.thw_cap = thw_cap
        self.thw_danger = thw_danger
        self.epsilon = epsilon

    def forward(
        self,
        attacker_pred: torch.Tensor,
        tgt_expanded: torch.Tensor,
    ) -> torch.Tensor:
        """
        计算 Time Headway 损失（对抗型：最小化 THW，将攻击者维持在 ego 近前方）。

        参数
        ────
        attacker_pred  : [NA-B, T, 4]  所有非 ego 车辆轨迹（UNNORMALIZED，已按图展开）
        tgt_expanded   : [NA-B, T, 4]  ego 轨迹（已按图对 NA-B 展开，与 attacker_pred 对齐）

        返回
        ────
        thw_loss : [NA-B, T-1]
            relu(THW - thw_danger)，满足以下约束：
            - 攻击者在 ego 后方            → 0（不惩罚，方向无效）
            - THW < thw_danger            → 0（危险前提已建立，不再推进）
            - thw_danger ≤ THW ≤ thw_cap → THW - thw_danger（梯度推动 THW 下降）
            - THW > thw_cap               → thw_cap - thw_danger（常数，无梯度）
        """
        # ── 1. 纵向相对距离（以 ego 视角投影）────────────────────────────
        # compute_longitudinal_metrics(ego, target) 返回：
        #   d_rel_long = (target_pos - ego_pos) · ego_heading
        #   > 0 代表 target（攻击者）在 ego 前方（符合前车场景）
        d_rel_long, _, _ = compute_longitudinal_metrics(
            tgt_expanded, attacker_pred,
            dt=self.dt, epsilon=self.epsilon,
        )  # [NA-B, T-1]

        # ── 2. ego 的纵向前向速度 ─────────────────────────────────────────
        ego_vel = (
            (tgt_expanded[:, 1:, :2] - tgt_expanded[:, :-1, :2]) / self.dt
        )  # [NA-B, T-1, 2]
        ego_h = tgt_expanded[:, :-1, 2:4]                                   # [NA-B, T-1, 2]
        ego_h_norm = ego_h / (torch.norm(ego_h, dim=2, keepdim=True) + self.epsilon)
        # 纵向速度标量（仅正向，负值=倒退不参与）
        v_ego = torch.relu(
            torch.sum(ego_vel * ego_h_norm, dim=2)
        ) + self.epsilon                                                     # [NA-B, T-1]

        # ── 3. THW = d_lon / v_ego（仅当攻击者在前方时有意义）──────────────
        thw_raw = d_rel_long / v_ego                                         # [NA-B, T-1]

        # 攻击者在 ego 后方（d_rel_long ≤ 0）→ 置 0，后续 relu 后损失为 0
        # 攻击者在前方但过远（thw_raw > thw_cap）→ 截断至 thw_cap，梯度消失
        thw_clamped = torch.where(
            d_rel_long > 0.0,
            torch.clamp(thw_raw, max=self.thw_cap),
            torch.zeros_like(thw_raw),
        )  # [NA-B, T-1]

        # NaN / Inf 保护（v_ego 贴近 epsilon 时仍可能出现极大值）
        thw_clamped = torch.nan_to_num(
            thw_clamped, nan=0.0, posinf=self.thw_cap, neginf=0.0
        )

        # ── 4. 损失 = relu(THW - thw_danger)：仅对"尚未危险"区间惩罚 ──────
        # 梯度下降 → 最小化损失 → 推动 THW 向 thw_danger 靠拢
        return torch.relu(thw_clamped - self.thw_danger)  # [NA-B, T-1]


class YawRateLoss(nn.Module):
    """
    Adversarial yaw-rate loss: encourages aggressive steering by returning
    (threshold - |yaw_rate|), clamped to [0, threshold].
    Gradient descent minimises this value → pushes |yaw_rate| above threshold.
    Once above threshold the loss is zero (goal achieved).
    """
    def __init__(self, dt=0.1, comfort_threshold_deg=15.0):
    ...
    def forward(self, future_pred):
        ...
        shortfall = torch.relu(self.comfort_threshold - abs_yaw_rate)
        return shortfall


class KinematicFeasibilityLoss(nn.Module):
    """
    运动学可行性软约束损失：对超出速度/加速度/Jerk 上限的轨迹施加 relu² 惩罚。

    设计原则
    ─────────
    - 软约束（relu²），不截断梯度，允许优化器在代价下降时越界
    - 为背景车辆和攻击车辆设置不同的物理上限（攻击车辆放宽）
    - 上限基于 NuScenes 城市驾驶统计 + 车辆物理极限：
        背景车辆: v_max=20 m/s, a_max=5 m/s², j_max=10 m/s³
        攻击车辆: v_max=30 m/s, a_max=9 m/s², j_max=18 m/s³
    """

    def __init__(
        self,
        v_max: float = 20.0,    # m/s（~72 km/h，城市场景上限）
        a_max: float = 5.0,     # m/s²（约 0.5g，急刹/急加速上限）
        j_max: float = 10.0,    # m/s³（舒适性上限）
        dt: float = 0.1,
    ):
        super().__init__()
        self.v_max = v_max
    ...
    def forward(self, traj: torch.Tensor) -> torch.Tensor:
        """
        Args:
            traj: [NA, T, 4]  (x, y, hx, hy) UNNORMALIZED 世界坐标（米）
        Returns:
            [NA] 每辆车的运动学违规惩罚（越大越违规）
        """
        NA, T, _ = traj.shape

        # ── Speed: ||Δpos|| / dt → [NA, T-1]
        speeds = torch.norm(traj[:, 1:, :2] - traj[:, :-1, :2], dim=2) / self.dt
        speed_viol = torch.relu(speeds - self.v_max).pow(2).mean(dim=1)   # [NA]

        if T < 3:
            return speed_viol

        # ── Acceleration: ||Δvel|| / dt → [NA, T-2]
        vel = (traj[:, 1:, :2] - traj[:, :-1, :2]) / self.dt              # [NA, T-1, 2]
        accels = torch.norm(vel[:, 1:] - vel[:, :-1], dim=2) / self.dt    # [NA, T-2]
        accel_viol = torch.relu(accels - self.a_max).pow(2).mean(dim=1)   # [NA]

        if T < 4:
            return speed_viol + accel_viol

        # ── Jerk: ||Δaccel_vec|| / dt → [NA, T-3]
        accel_vec = (vel[:, 1:] - vel[:, :-1]) / self.dt                   # [NA, T-2, 2]
        jerks = torch.norm(accel_vec[:, 1:] - accel_vec[:, :-1], dim=2) / self.dt  # [NA, T-3]
        jerk_viol = torch.relu(jerks - self.j_max).pow(2).mean(dim=1)     # [NA]

        return speed_viol + accel_viol + jerk_viol

    @torch.no_grad()
    def compute_stats(self, traj: torch.Tensor):
        """
        不计算梯度，返回详细统计值供 FeasibilityReporter 使用。
        Returns dict: {speeds, accels, jerks} 每项均为 [NA, T-k] 张量
        """
        NA, T, _ = traj.shape
        speeds = torch.norm(traj[:, 1:, :2] - traj[:, :-1, :2], dim=2) / self.dt
        vel = (traj[:, 1:, :2] - traj[:, :-1, :2]) / self.dt
        accels = torch.norm(vel[:, 1:] - vel[:, :-1], dim=2) / self.dt if T >= 3 else None
        if T >= 4:
            accel_vec = (vel[:, 1:] - vel[:, :-1]) / self.dt
            jerks = torch.norm(accel_vec[:, 1:] - accel_vec[:, :-1], dim=2) / self.dt
        else:
            jerks = None
        return {"speeds": speeds, "accels": accels, "jerks": jerks}
