...
def viz_scene_graph(scene_graph, map_idx, map_env, bidx, out_path,
    ...
    '''
    # 从数据集中获取dt
    dt = scene_graph.dt if hasattr(scene_graph, 'dt') else 0.5  # 默认值为0.5
...
def create_video(img_path_form, out_path, fps):
    ...
    import subprocess
    import logging
    command = ['ffmpeg', '-y', '-r', str(fps), '-i', img_path_form,
    ...
    try:
        ...
        logging.error("ffmpeg not found. Please ensure ffmpeg is installed and in your PATH.")
        return  # or raise an exception
    except subprocess.CalledProcessError as e:
        # 如果ffmpeg返回非零退出码，记录错误
...
def viz_map_crop_video(x, outdir, car_kin=None, car_lw=None, viz_traj=False, indiv=True,
                        car_colors=None, car_alpha=None, fps=2, save_timestamps=None, dt=0.1,
                        keep_frames=False):
    '''
    ...
    for s in range(NS):
        ...
        for t in range(NT):
            frame_path = os.path.join(cur_outdir, 'frame%04d.png' % (t))
            viz_map_crop(x, frame_path,
                        car_kin=car_kin[:, s:s+1, t:t+1, :], car_lw=car_lw, 
                        viz_traj=viz_traj, indiv=indiv, car_colors=car_colors,
                        car_alpha=car_alpha)
            
            # Save specific frames if requested
            if save_timestamps is not None:
                current_time = t * dt
                # Check if current_time is close to any of the requested timestamps
                for ts in save_timestamps:
                    if abs(current_time - ts) < (dt / 2.0):
                        save_path = os.path.join(os.path.dirname(outdir), f'{os.path.basename(outdir)}_at_{ts:.1f}s.png')
                        shutil.copy(frame_path, save_path)
                        print(f'Saved frame at {current_time:.1f}s to {save_path}')
...
def viz_map_crop(x, out_path, car_kin=None, car_lw=None, gt_kin=None, viz_traj=False, indiv=True,
    ...
    '''
    def style_ax():
        # Disable grid explicitly for newer Matplotlib APIs
        plt.grid(False)
        plt.xticks([])
        plt.yticks([])
...
def render_map_observation(x):
    ...
    map_color_list = ['grey', 'coral', '#FFAA00', '#E0E0E0', 'lightblue', 'lightblue']
    map_alpha_list = [1.0, 0.6, 0.8, 0.8, 1.0, 0.5]
...
def render_obj_observation(car_kin, car_lw, viz_traj=False, viz_init_car=True, color=None,
    ...
    """
    :param show_vehicle_id: 是否在车辆附近显示车辆ID标签
    """
        ...
        if viz_traj:
            ...
            if len(car_kin.size()) == 3:
                ...
                for n in range(NA):
                    ...
                    NT = cur_traj.size(0)
                    cur_alph = alpha[n] if alpha is not None else 0.8
                    cur_linewidth = traj_linewidth[n] if traj_linewidth is not None else 1.5
            ...
            if viz_init_car:
                ...
                if len(car_kin.size()) == 3:
                    ...
                    for n in range(NA):
                        ...
                        nonnan_indices = torch.nonzero(~torch.isnan(car_kin[n, :, 0]), as_tuple=True)[0]
                        if nonnan_indices.numel() > 0:
                            nonnan_idx = nonnan_indices[0]
                            ...
                            if show_vehicle_id:
                                label_text = f'EGO' if n == 0 else f'V{n}'
                                plt.text(car_kin[n, nonnan_idx, 0].item(), car_kin[n, nonnan_idx, 1].item(), 
                                        label_text, fontsize=12, fontweight='bold',
                                        color='white', ha='center', va='center',
                                        bbox=dict(boxstyle='round,pad=0.4', facecolor=cur_col, alpha=0.9, edgecolor='white', linewidth=2))
                elif len(car_kin.size()) == 4:
                    ...
                    for n in range(NA):
                        for s in range(1):
                            cur_col = color[n] if color is not None else plt_color(n % 9)
                            cur_alph = alpha[n] if alpha is not None else 0.7
                            nonnan_indices = torch.nonzero(~torch.isnan(car_kin[n, s, :, 0]), as_tuple=True)[0]
                            if nonnan_indices.numel() > 0:
                                nonnan_idx = nonnan_indices[0]
                                plot_car(car_kin[n, s, nonnan_idx, 0], car_kin[n, s, nonnan_idx, 1], car_kin[n, s, nonnan_idx, 2], car_kin[n, s, nonnan_idx, 3],
                                            car_lw[n, 0], car_lw[n, 1], cur_col, alpha=cur_alph)
                                # Add vehicle ID label
                                if show_vehicle_id and s == 0:  # Only show label once per vehicle
                                    label_text = f'EGO' if n == 0 else f'V{n}'
                                    plt.text(car_kin[n, s, nonnan_idx, 0].item(), car_kin[n, s, nonnan_idx, 1].item(), 
                                            label_text, fontsize=12, fontweight='bold',
                                            color='white', ha='center', va='center',
                                            bbox=dict(boxstyle='round,pad=0.4', facecolor=cur_col, alpha=0.9, edgecolor='white', linewidth=2))
        else:
