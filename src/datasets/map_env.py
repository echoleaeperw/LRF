...
class NuScenesMapEnv(object):
    def __init__(self, map_data_path,
        ...
        # Cache vector line data for BEV rendering
        self._vector_cache = self._build_vector_cache()

    @staticmethod
    def _offset_polyline(pts, offset):
        """Offset a polyline by *offset* metres perpendicular to each segment.

        Positive offset = left side (counter-clockwise normal).
        Returns an array of the same shape as *pts*.
        """
        tangents = np.diff(pts, axis=0)
        tangents = np.vstack([tangents, tangents[-1:]])
        lengths = np.linalg.norm(tangents, axis=1, keepdims=True).clip(1e-6)
        tangents = tangents / lengths
        normals = np.column_stack([-tangents[:, 1], tangents[:, 0]])
        return pts + normals * offset

    def _build_vector_cache(self):
        """Extract polyline geometry from nuScenes map API for all maps.

        For each lane, discretizes the centerline at 1 m resolution and
        generates left/right boundary lines by offsetting the centerline by
        half the estimated lane width (~1.8 m).  This produces dense,
        continuous lane boundary polylines that look much better than the
        sparse ``lane_divider`` node records.
        """
        from nuscenes.map_expansion.arcline_path_utils import discretize_lane
        HALF_LANE_W = 1.8
        cache = {}
        for mname, nmap in self.nusc_maps.items():
            print(f'  Building vector cache for {mname}...')
            is_singapore = mname.split('-')[0] == 'singapore'
            mheight = NUSC_MAP_SIZES[mname][0]
            lines = {
                'road_divider': [],
                'lane_divider': [],
            ...
            def _flip_y(pts):
                if self.flip_singapore and is_singapore:
                    pts = pts.copy()
                    pts[:, 1] = mheight - pts[:, 1]
                return pts

            def _get_line_coords(node_tokens):
                coords = []
                for nt in node_tokens:
                    nd = nmap.get('node', nt)
                    coords.append((nd['x'], nd['y']))
                pts = np.array(coords) if coords else np.empty((0, 2))
                return _flip_y(pts)

            for rd in nmap.road_divider:
                pts = _get_line_coords(rd['node_tokens'])
                if len(pts) >= 2:
                    lines['road_divider'].append(pts)

            for ld in nmap.lane_divider:
                seg_types = [s['segment_type'] for s in ld.get('lane_divider_segments', [])]
                pts = _get_line_coords(ld['node_tokens'])
                if len(pts) >= 2:
                    dominant = max(set(seg_types), key=seg_types.count) if seg_types else 'SINGLE_SOLID_WHITE'
                    lines['lane_divider'].append({'pts': pts, 'type': dominant})

            lane_tokens = [rec['token'] for rec in nmap.lane]
            for lt in lane_tokens:
                try:
                    arcline = nmap.get_arcline_path(lt)
                    pts_3d = np.array(discretize_lane(arcline, resolution_meters=1.0))
                    pts_2d = _flip_y(pts_3d[:, :2])
                    if len(pts_2d) < 2:
                        continue
                    lines['lane_centerline'].append(pts_2d)
                    left = self._offset_polyline(pts_2d, HALF_LANE_W)
                    right = self._offset_polyline(pts_2d, -HALF_LANE_W)
                    lines['lane_boundary'].append(left)
                    lines['lane_boundary'].append(right)
                except Exception:
                    pass

            for da in nmap.drivable_area:
                for poly_token in da['polygon_tokens']:
                    poly = nmap.get('polygon', poly_token)
                    ext_pts = _get_line_coords(poly['exterior_node_tokens'])
                    if len(ext_pts) >= 3:
                        ext_pts = np.vstack([ext_pts, ext_pts[0:1]])
                        lines['drivable_boundary'].append(ext_pts)

            print(f'    lanes={len(lines["lane_centerline"])}, '
                  f'boundaries_from_lanes={len(lines["lane_boundary"])}, '
                  f'road_div={len(lines["road_divider"])}, '
                  f'drivable={len(lines["drivable_boundary"])}')
            cache[mname] = lines
        return cache

    def get_vector_lines_in_crop(self, center_pos, map_idx_val, bounds=None, L=None, W=None):
        """
        Return vector lines clipped and transformed into crop pixel coordinates.

        :param center_pos: (4,) tensor [x, y, hx, hy] — crop center in global map coords
        :param map_idx_val: int — index into self.map_list
        :param bounds: [low_l, low_w, high_l, high_w]
        :returns: dict with keys 'road_divider', 'lane_divider', 'drivable_boundary',
                  each a list of dicts {'pts': np.array (N,2) in pixel coords, ...}
        """
        bounds = self.bounds if bounds is None else bounds
        L = self.L if L is None else L
        W = self.W if W is None else W

        mname = self.map_list[map_idx_val]
        vec_data = self._vector_cache[mname]

        cx, cy = float(center_pos[0]), float(center_pos[1])
        hx, hy = float(center_pos[2]), float(center_pos[3])
        ...
        pix_per_m_L = L / float(bounds[2] - bounds[0])
        pix_per_m_W = W / float(bounds[3] - bounds[1])

        def _to_crop_pix(pts_global):
            dx = pts_global[:, 0] - cx
            dy = pts_global[:, 1] - cy
            local_x = dx * cos_h - dy * sin_h
            local_y = dx * sin_h + dy * cos_h
            pix_x = (local_x - bounds[0]) * pix_per_m_L
            pix_y = (local_y - bounds[1]) * pix_per_m_W
            return np.stack([pix_x, pix_y], axis=1)

        def _in_bbox(pts):
            return np.any(
                (pts[:, 0] >= bbox_min_x) & (pts[:, 0] <= bbox_max_x) &
                (pts[:, 1] >= bbox_min_y) & (pts[:, 1] <= bbox_max_y)
            )

        result = {
            'road_divider': [], 'lane_divider': [],
            'lane_centerline': [], 'lane_boundary': [],
            'drivable_boundary': [],
        }

        for pts in vec_data['road_divider']:
            if _in_bbox(pts):
                result['road_divider'].append({'pts': _to_crop_pix(pts), 'type': 'solid_yellow'})

        for ld_item in vec_data['lane_divider']:
            pts = ld_item['pts']
            if _in_bbox(pts):
                result['lane_divider'].append({'pts': _to_crop_pix(pts), 'type': ld_item['type']})

        for pts in vec_data.get('lane_centerline', []):
            if _in_bbox(pts):
                result['lane_centerline'].append({'pts': _to_crop_pix(pts)})

        for pts in vec_data.get('lane_boundary', []):
            if _in_bbox(pts):
                result['lane_boundary'].append({'pts': _to_crop_pix(pts)})

        for pts in vec_data['drivable_boundary']:
            if _in_bbox(pts):
                result['drivable_boundary'].append({'pts': _to_crop_pix(pts)})
