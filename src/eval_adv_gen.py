...
from eval_adversarial_trajectory import AdversarialTrajectoryEvaluator
...
def parse_cfg():
    ...
    parser.add_argument('--scenarios', type=str, required=True,
                        help='Directory to load scenarios from, should contain low_risk, high_risk, and longtail_condition')
    ...
    # viz options
    parser.add_argument('--viz_res', type=str, nargs='+', default=['high_risk'], 
                        choices=['low_risk', 'high_risk', 'longtail_condition'], help='Which results to visualize.')
...
def plot_scenario_distrib(clustering, cluster_labels, high_risk, longtail_condition, out_path):
    ...
    for cur_scenes in [high_risk, longtail_condition]: # plot each directory independently
...
def quant_eval(scenarios, cluster_path, cluster_labels, map_env, out_path):
    ...
    coll_feat_list = []
    for coll_sname in ['high_risk', 'longtail_condition']:
    ...
    # plot label distribution comparison high risk and longtail
    plot_scenario_distrib(clustering, cluster_labels, scenarios['high_risk'], scenarios['longtail_condition'],
                            os.path.join(out_path, 'scene_distrib.png'))

    low_risk_rate, high_risk_rate, longtail_rate = compute_risk_distribution(scenarios)
    print(f"风险分布: 低风险={low_risk_rate:.2%}, 高风险={high_risk_rate:.2%}, 长尾={longtail_rate:.2%}")

    metrics = {}
    freq_metrics_cnt = {}
    freq_metrics_total = {}
    # accumulate scenarios starting from longtail, then high risk, then low risk scenarios
    coll_sidx = 0
    resnames = ['longtail_condition', 'high_risk', 'low_risk']
    for residx, resname in enumerate(resnames):
        for scene in scenarios[resname]:
            ...
            # 为新的风险分类系统添加风险等级标记
            if resname in ['longtail_condition', 'high_risk']:
                risk_level = 2 if resname == 'longtail_condition' else 1  # longtail=2, high_risk=1, low_risk=0
                freq_metrics_cnt, freq_metrics_total = log_freq_stat(freq_metrics_cnt, freq_metrics_total, 
                                                                    'risk_level', risk_level, 1)
                seq_metrics['risk_level'] = risk_level
                coll_sidx += 1
            else:
                seq_metrics['risk_level'] = 0  # low_risk

            scene['eval_metrics'] = seq_metrics

        eval_name = None
        if resname == 'longtail_condition':
            eval_name = 'longtail'
        elif resname == 'high_risk':
            eval_name = 'high_risk'
        elif resname == 'low_risk':
            eval_name = 'all_scenes'
        # per-seq csv
        # 获取第一个非空场景的指标键名
        first_scenario_key = None
        for key in ['longtail_condition', 'high_risk', 'low_risk']:
            if len(scenarios[key]) > 0:
                first_scenario_key = key
                break
        
        if first_scenario_key:
            per_seq_metrics = sorted(list(scenarios[first_scenario_key][0]['eval_metrics'].keys()))
        else:
            per_seq_metrics = []
...
def compute_risk_distribution(scenarios):
    """计算风险等级分布"""
    tot_scenes = float(sum([len(cur_scenes) for _, cur_scenes in scenarios.items()]))
    if tot_scenes == 0:
        return 0.0, 0.0, 0.0
    
    low_risk_rate = len(scenarios['low_risk']) / tot_scenes
    high_risk_rate = len(scenarios['high_risk']) / tot_scenes  
    longtail_rate = len(scenarios['longtail_condition']) / tot_scenes
    return low_risk_rate, high_risk_rate, longtail_rate
    ...
    for resname in viz_res:
        ...
        for scene in scenarios[resname]:
            ...
            for stage in viz_stage:
                if resname == 'low_risk' and stage == 'sol':
                    continue # can't viz this for low risk scenarios
...
def eval_adv_gen(cfg):
    ...
    res_names = ['low_risk', 'high_risk', 'longtail_condition']
    ...
    # 添加长尾事件分析
    if hasattr(cfg, 'eval_longtail') and cfg.eval_longtail:
        longtail_out = os.path.join(out_path, 'eval_longtail')
        mkdir(longtail_out)
        longtail_eval(scenarios, longtail_out)
