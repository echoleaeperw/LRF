...
# 文件位于 src/utils/，需要将 src/ 加入路径以导入 losses/utils 等子包
_this_dir = os.path.abspath(os.path.dirname(__file__))
_src_root = os.path.abspath(os.path.join(_this_dir, '..'))      # STRIVE/src/
_md_root  = os.path.abspath(os.path.join(_this_dir, '..', '..', '..', 'MD'))  # longtail_coverage_analyzer 所在目录
...
class AdversarialTrajectoryEvaluator:
    ...
    def __init__(self, device='cuda', verbose=True, dt=0.1):
        self.device = device
        self.verbose = verbose
        self.dt = dt  # 时间步长，由外部传入（JSON 数据为 0.5s）
    ...
    def _evaluate_longtail_coverage(self, 
        ...
        # 6. 新增：基于物理特征的覆盖率分析（传入 dt 确保速度/加速度计算正确）
        try:
    ...
    def _check_physics_constraints(self, trajectories: torch.Tensor) -> float:
        ...
        # 计算速度和加速度
        velocities, speeds = compute_velocities_from_traj(trajectories, dt=self.dt)
        ...
        # 2. 加速度约束检查
        if T > 2:
            accelerations = (speeds[:, 1:] - speeds[:, :-1]) / self.dt
        ...
        if T > 1:
            ...
            yaw_rates = torch.abs(yaw_angles[:, 1:] - yaw_angles[:, :-1]) / self.dt
    ...
    def _compute_relative_velocities(self, trajectories: torch.Tensor) -> torch.Tensor:
        """计算相对速度"""
        velocities, _ = compute_velocities_from_traj(trajectories, dt=self.dt)
    ...
    def _identify_rare_events(self, traj_adversarial: torch.Tensor, 
        ...
        # 1. 极端加速度事件
        velocities, speeds = compute_velocities_from_traj(traj_adversarial, dt=self.dt)
        accelerations = (speeds[:, 1:] - speeds[:, :-1]) / self.dt
    ...
    def _detect_extreme_behaviors(self, trajectories: torch.Tensor) -> float:
        ...
        # 2. 急刹车
        if speeds.shape[1] > 1:
            decelerations = -(speeds[:, 1:] - speeds[:, :-1]) / self.dt
    ...
    def _compute_distribution_gap(self, traj_sim: torch.Tensor, 
        ...
        """计算分布差异"""
        # 提取速度分布
        _, speeds_sim = compute_velocities_from_traj(traj_sim, dt=self.dt)
        _, speeds_real = compute_velocities_from_traj(traj_real, dt=self.dt)
    ...
    def _compute_motion_feature_gap(self, traj_sim: torch.Tensor, 
        ...
        if speeds_sim.shape[1] > 1:
            accel_sim = (speeds_sim[:, 1:] - speeds_sim[:, :-1]) / self.dt
            accel_real = (speeds_real[:, 1:] - speeds_real[:, :-1]) / self.dt
    ...
    def _compute_detailed_metrics(self, 
        ...
        # 轨迹分析
        velocities_orig, speeds_orig = compute_velocities_from_traj(traj_original, dt=self.dt)
        velocities_adv, speeds_adv = compute_velocities_from_traj(traj_adversarial, dt=self.dt)
    ...
    def analyze_longtail_events_8s(self, 
        ...
        判定逻辑（AND）：对任意车辆对 (i, j)，当以下三个条件同时满足时认定为长尾事件：
            TTC  < 1.5 s  —— 碰撞时间极短
            TLC  < 0.8 s  —— 横向越道时间极短（代理值：假设车道半宽 1.75m）
            THW  < 1.0 s  —— 跟车时距极短

        参数:
            trajectories:   车辆轨迹 [NA, T, 4]  (x, y, cos_h, sin_h)
            vehicle_lengths: 车辆长度 [NA]（None → 默认 4.5 m）
            vehicle_widths:  车辆宽度 [NA]（None → 默认 2.0 m）
            dt:              时间步长（秒），默认 0.5 s

        返回:
            Dict 包含：
            - has_longtail_event:    bool，是否发生长尾事件
            - longtail_event_types:  List[str]，触发的事件类型
            - event_details:         Dict，各指标事件详情
            - vehicle_event_flags:   torch.Tensor [NA]，每辆车是否参与长尾事件
            - detection_criteria:    Dict，使用的阈值和逻辑说明
        """
        NA, T, _ = trajectories.shape
        ...
        tlc_threshold = 0.8   # s
        thw_threshold = 1.0   # s
        assumed_half_lane = 1.75  # 代理车道半宽（m），无地图时使用

        # ── 内部辅助：TLC 代理（无地图版）──────────────────────────────────
        def _min_tlc_proxy(traj_v: torch.Tensor) -> float:
            """返回 traj_v 在整条轨迹上的最小 TLC（代理值）"""
            min_tlc = float('inf')
            for t in range(T - 1):
                hx = traj_v[t, 2].item()
                hy = traj_v[t, 3].item()
                h_norm = (hx**2 + hy**2)**0.5 + 1e-6
                hx, hy = hx / h_norm, hy / h_norm
                # 横向方向单位向量（左侧为正）
                lat_x, lat_y = -hy, hx
                vx = (traj_v[t + 1, 0] - traj_v[t, 0]).item() / dt
                vy = (traj_v[t + 1, 1] - traj_v[t, 1]).item() / dt
                v_lat = abs(vx * lat_x + vy * lat_y)
                if v_lat > 0.1:
                    tlc = assumed_half_lane / v_lat
                    if tlc < min_tlc:
                        min_tlc = tlc
            return min_tlc

        # ── 内部辅助：THW（atk 在 ego 前方时的时距）────────────────────────
        def _min_thw(traj_ego: torch.Tensor, traj_atk: torch.Tensor) -> float:
            """返回 ego 跟随 atk 时的最小 THW = d_lon / v_ego"""
            min_thw = float('inf')
            for t in range(T - 1):
                hx = traj_ego[t, 2].item()
                hy = traj_ego[t, 3].item()
                h_norm = (hx**2 + hy**2)**0.5 + 1e-6
                hx, hy = hx / h_norm, hy / h_norm
                rel_x = (traj_atk[t, 0] - traj_ego[t, 0]).item()
                rel_y = (traj_atk[t, 1] - traj_ego[t, 1]).item()
                d_lon = rel_x * hx + rel_y * hy  # > 0 表示 atk 在前
                if d_lon > 0:
                    vx = (traj_ego[t + 1, 0] - traj_ego[t, 0]).item() / dt
                    vy = (traj_ego[t + 1, 1] - traj_ego[t, 1]).item() / dt
                    v_ego = (vx**2 + vy**2)**0.5
                    if v_ego > 0.5:
                        thw = d_lon / v_ego
                        if thw < min_thw:
        ...
        longtail_event_types: List[str] = []
        event_details: Dict[str, List] = {
            'longtail_events': [],   # AND 条件同时满足的事件
            'ttc_events': [],        # 单项参考
            'tlc_events': [],
            'thw_events': [],
        }
        ...
        for i in range(NA):
            for j in range(i + 1, NA):
                ...
                # ── 计算该车对的最小 TTC ───────────────────────────────────
                min_ttc_pair = float('inf')
                for t in range(T - 1):
                    pos_i = traj_i[t, :2]
                    pos_j = traj_j[t, :2]
                    vel_i = (traj_i[t + 1, :2] - traj_i[t, :2]) / dt
                    vel_j = (traj_j[t + 1, :2] - traj_j[t, :2]) / dt
                    rel_pos = pos_j - pos_i
                    rel_vel = vel_j - vel_i
                    rel_speed_sq = torch.sum(rel_vel * rel_vel).item()
                    if rel_speed_sq > 1e-6:
                        dot = torch.sum(rel_pos * rel_vel).item()
                        if dot < 0:
                            dist = torch.norm(rel_pos).item()
                            ttc = dist / (rel_speed_sq**0.5)
                            if ttc < min_ttc_pair:
                                min_ttc_pair = ttc

                # ── 计算该车对的最小 TLC（取双方较小值）──────────────────
                min_tlc_i = _min_tlc_proxy(traj_i)
                min_tlc_j = _min_tlc_proxy(traj_j)
                min_tlc_pair = min(min_tlc_i, min_tlc_j)

                # ── 计算该车对的最小 THW（双向取小）──────────────────────
                thw_ij = _min_thw(traj_i, traj_j)   # j 在 i 前方
                thw_ji = _min_thw(traj_j, traj_i)   # i 在 j 前方
                min_thw_pair = min(thw_ij, thw_ji)

                # ── 各指标单项记录（供调试/分析使用）──────────────────────
                if min_ttc_pair < ttc_threshold:
                    event_details['ttc_events'].append({
                        'vehicle_pair': (i, j),
                        'min_ttc': round(min_ttc_pair, 4),
                    })
                if min_tlc_pair < tlc_threshold:
                    event_details['tlc_events'].append({
                        'vehicle_pair': (i, j),
                        'min_tlc': round(min_tlc_pair, 4),
                    })
                if min_thw_pair < thw_threshold:
                    event_details['thw_events'].append({
                        'vehicle_pair': (i, j),
                        'min_thw': round(min_thw_pair, 4),
                    })

                # ── AND 判定：三项同时满足才算长尾事件 ────────────────────
                if (min_ttc_pair < ttc_threshold and
                        min_tlc_pair < tlc_threshold and
                        min_thw_pair < thw_threshold):
                    longtail_event_types.append('longtail_condition')
                    event_details['longtail_events'].append({
                        'vehicle_pair': (i, j),
                        'min_ttc': round(min_ttc_pair, 4),
                        'min_tlc': round(min_tlc_pair, 4),
                        'min_thw': round(min_thw_pair, 4),
        ...
        return {
            ...
            'total_events': len(event_details['longtail_events']),
            'detection_criteria': {
                'ttc_threshold': ttc_threshold,
                'tlc_threshold': tlc_threshold,
                'thw_threshold': thw_threshold,
                'logic': 'AND (TTC < 1.5s AND TLC < 0.8s AND THW < 1.0s)',
            },
        }
    ...
    def analyze_longtail_events_8s_enhanced(self,
                                          ...
                                          dt: float = 0.5) -> Dict[str, Any]:
        """
        增强版长尾事件分析函数（使用 compute_longitudinal_metrics 的精确 TTC）。

        判定逻辑（AND）：对任意车辆对 (i, j)，当以下三个条件同时满足时认定为长尾事件：
            TTC  < 1.5 s  —— 纵向碰撞时间极短（来自 compute_longitudinal_metrics）
            TLC  < 0.8 s  —— 横向越道时间极短（代理值：假设车道半宽 1.75 m）
            THW  < 1.0 s  —— 跟车时距极短（d_lon / v_ego）
        ...
        for i in range(NA):
            for j in range(i + 1, NA):
                ...
                # ── 精确 TTC 和 THW（via compute_longitudinal_metrics）────
                min_ttc_pair = float('inf')
                min_thw_pair = float('inf')
                try:
                    d_rel_long, v_rel_long, ttc = compute_longitudinal_metrics(
                        traj_i, traj_j, dt=dt, epsilon=1e-6
                    )
                    if ttc.numel() > 0:
                        ttc_1d        = ttc.squeeze(0)          # [T-1]
                        d_rel_long_1d = d_rel_long.squeeze(0)   # [T-1]
                        v_rel_long_1d = v_rel_long.squeeze(0)   # [T-1]

                        # 最小有效 TTC
                        valid_mask = ~torch.isinf(ttc_1d) & (ttc_1d > 0)
                        if valid_mask.any():
                            min_ttc_pair = ttc_1d[valid_mask].min().item()

                        # THW = d_lon / v_ego（仅当 d_lon > 0，即 j 在 i 前方）
                        # v_ego 用 traj_i 速度大小近似
                        ego_pos = trajectories[i, :, :2]
                        ego_speed = torch.norm(
                            (ego_pos[1:] - ego_pos[:-1]) / dt, dim=1
                        )  # [T-1]
                        fwd_mask = (d_rel_long_1d > 0) & (ego_speed > 0.5)
                        if fwd_mask.any():
                            thw_vals = d_rel_long_1d[fwd_mask] / (ego_speed[fwd_mask] + 1e-6)
                            min_thw_pair = thw_vals.min().item()

                        # 同样检查反向（i 在 j 前方）
                        d_rel_long_r, _, ttc_r = compute_longitudinal_metrics(
                            traj_j, traj_i, dt=dt, epsilon=1e-6
                        )
                        if ttc_r.numel() > 0:
                            d_r_1d = d_rel_long_r.squeeze(0)
                            atk_pos = trajectories[j, :, :2]
                            atk_speed = torch.norm(
                                (atk_pos[1:] - atk_pos[:-1]) / dt, dim=1
                            )
                            fwd_r = (d_r_1d > 0) & (atk_speed > 0.5)
                            if fwd_r.any():
                                thw_r_vals = d_r_1d[fwd_r] / (atk_speed[fwd_r] + 1e-6)
                                min_thw_pair = min(min_thw_pair,
                                                   thw_r_vals.min().item())

                except Exception as e:
                    Logger.log(f"纵向指标计算失败 (车辆 {i}, {j}): {e}")

                # ── TLC 代理（双方取小）────────────────────────────────────
                min_tlc_pair = min(
                    _min_tlc_proxy(trajectories[i]),
                    _min_tlc_proxy(trajectories[j]),
                )
        ...
        return {
            ...
            'analysis_summary': {
                'longtail_events_count': len(event_details['longtail_events']),
                'ttc_events_count':      len(event_details['ttc_events']),
                'tlc_events_count':      len(event_details['tlc_events']),
                'thw_events_count':      len(event_details['thw_events']),
...
def example_longtail_analysis():
    ...
    print(f"判定标准: {basic_results['detection_criteria']['logic']}")
    
    if basic_results['event_details']['longtail_events']:
        print("长尾事件详情（AND 条件同时满足）:")
        for event in basic_results['event_details']['longtail_events']:
            print(f"  车辆对 {event['vehicle_pair']}: "
                  f"TTC={event['min_ttc']:.3f}s  TLC={event['min_tlc']:.3f}s  THW={event['min_thw']:.3f}s")
    ...
    print(f"判定标准: {enhanced_results['detection_criteria']['logic']}")
    print(f"分析摘要:")
    for key, value in enhanced_results['analysis_summary'].items():
        print(f"  {key}: {value}")
    
    if enhanced_results['event_details']['longtail_events']:
        print("长尾事件详情（AND 条件同时满足）:")
        for event in enhanced_results['event_details']['longtail_events']:
            print(f"  车辆对 {event['vehicle_pair']}: "
                  f"TTC={event['min_ttc']:.3f}s  TLC={event['min_tlc']:.3f}s  THW={event['min_thw']:.3f}s")
