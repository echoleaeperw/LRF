...
def run_find_solution_optim(cur_z, final_result_traj, future_len, lr, loss_weights, model, 
    ...
    for oidx in pbar_optim:
        def closure():
            ...
            # 获取其他车辆轨迹，用于TTC等交互损失的计算
            other_future_unnorm = model.get_normalizer().unnormalize(match_decoder_out['future_pred'][~tgt_mask])
            
            # 计算避撞损失，并传递其他车辆轨迹作为目标
            tgt_avoid_loss_dict = avoid_loss(tgt_future_pred,
                                            tgt_z,
                                            tgt_prior_distrib,
                                            tgt_traj=other_future_unnorm)  # 传递目标轨迹
